...
main(int argc,
  ...
  if (feature || automatic) {
    std::cout << "creating feature set" << std::endl;

    FeatureSet featureSet;

    std::printf("automatically generating features\n");
    featureSet.autoDetectFeatures(trimesh, 110.0);
...
//  }
  ...
  TetMeshUGridWriter(mesh, filenameStr + ".ugrid");

  // Write out an "info" file for the mesh as well.
  std::stringstream ss;
  ss << filenameStr << ".info";
  std::printf("Writing mesh info to file: %s\n", ss.str().c_str());
  bool result = mesh.writeInfoToFile(ss.str().c_str());
  if (!result) {
